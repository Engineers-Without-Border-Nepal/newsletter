...
function personalizeEmailContent(htmlContent, userId) {
  return htmlContent.replace(
    /userid=409238740234/g,
    `userid=${userId}`
  );
}

const logFilePath = path.join(__dirname, "email_stars_log.txt");
...
async function main() {
  ...
  for (const subscriber of subscribers) {
    try {
      const personalizedContent = personalizeEmailContent(htmlContent, subscriber.userId);
      
      await transporter.sendMail({
        from: '"EWB Nepal" <contact@ewb.org.np>',
        to: subscriber.email,
        subject: "EWBN Newsletter | June Edition",
        html: personalizedContent,
      });

      console.log(`${subscriber.email} - success`);
